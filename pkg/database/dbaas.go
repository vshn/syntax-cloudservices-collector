...
func (d *DBaaSDatabase) ensureAggregatedServiceUsage(ctx context.Context, aggregatedDatabaseService Aggregated, s *reporting.Store) error {
	...
	return s.WithTransaction(ctx, func(tx *sqlx.Tx) error {
		tenant, err := tenantsmodel.Ensure(ctx, tx, &db.Tenant{Source: aggregatedDatabaseService.Organization})
		if err != nil {
		...
		product, err := productsmodel.GetBestMatch(ctx, tx, sourceString.getSourceString(), d.BillingDate)
		if err != nil {
			return fmt.Errorf("cannot get product best match for namespace %s: %w", namespace, err)
		}

		discount, err := discountsmodel.GetBestMatch(ctx, tx, sourceString.getSourceString(), d.BillingDate)
		if err != nil {
		...
		storageFact := factsmodel.New(dateTime, query, tenant, category, product, discount, aggregatedDatabaseService.Value)
...
func isFactsUpdatable(ctx context.Context, tx *sqlx.Tx, f *db.Fact, value float64) bool {
	log := ctrl.LoggerFrom(ctx)
	fact, _ := factsmodel.GetByFact(ctx, tx, f)
